-- mvp_pilot_staff_create_task_returning_alias_v2_1h.sql
-- Fixes confirmed SQLSTATE 42702 ("column reference \"task_number\" is
-- ambiguous") in public.staff_create_task's INSERT ... RETURNING clause.
-- Root cause: RETURNS TABLE(task_id uuid, task_number text, bridge_id uuid,
-- bridge_number text) implicitly declares a PL/pgSQL OUT variable named
-- task_number, which collides with the unqualified staff_tasks.task_number
-- column referenced in RETURNING id, task_number, is_bridge.
--
-- Minimum fix: alias the INSERT target and qualify the three RETURNING
-- columns against that alias. No other character of the function body is
-- changed: same signature, same RETURNS TABLE, same SECURITY DEFINER, same
-- search_path, same validation/authorization branches, same INSERT values,
-- same Bridge/notification/audit logic. CREATE OR REPLACE FUNCTION preserves
-- the function's existing owner and grants unchanged.
...
BEGIN
  ...
  INSERT INTO public.staff_tasks AS inserted_task (
    ...
    v_caller, p_reference_number, p_due_date, p_due_time

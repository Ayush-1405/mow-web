...
// Stock Transfer Requests — deliberately NOT a new table. Reuses the
// existing generic staff_tasks/bridges engine (the same staff_create_task
// RPC AssignTask.jsx already uses), filtered to the STOCK_TRANSFER task
// type added in mvp_pilot_retail_task_types_v2_2d.sql, so this is the same
// working Accept/Return/Start/Complete/Verify/Close flow every other
// cross-department request already goes through — not a duplicate.
export default function RetailStockTransfer({ lang, profile, lookups }) {
  ...
  const [form, setForm] = useState({ title: "", assigned_to: "", due_date: "", quantity: "" });

  const godownDept = useMemo(() => lookups.departments.find((d) => d.code === "GODOWN_INV"), [lookups.departments]);
  const taskType = useMemo(() => lookups.taskTypes.find((tt) => tt.code === "STOCK_TRANSFER"), [lookups.taskTypes]);
  ...
  const load = useCallback(async () => {
    ...
    setUsers((usersRes.data || []).filter((u) => u.department_id === godownDept?.id));
    setLoading(false);
  }, [taskType, godownDept]);
  ...
  async function handleRequest(e) {
    e.preventDefault();
    if (!form.title || !form.assigned_to || !form.due_date || !godownDept || !taskType) return;
    setSaving(true);
    const { error: err } = await supabase.rpc("staff_create_task", {
      ...
      p_to_department_id: godownDept.id,
    ...
    setForm({ title: "", assigned_to: "", due_date: "", quantity: "" });
    setShowForm(false);
    load();
  }
  ...
  return (
    <div className="dept-dashboard">
      <div className="dept-header card">
        ...
        <div className="dept-header-text"><h1>{t("retailStockTransferTitle", lang)}</h1></div>
      </div>

      <div className="card">
        <button className="btn btn-primary" onClick={() => setShowForm((s) => !s)} disabled={!taskType}>
          {showForm ? t("cancel", lang) : t("requestTransfer", lang)}
        </button>
        {showForm && (

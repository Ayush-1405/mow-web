-- mvp_pilot_fix_bridge_insert_ambiguous_column_v2_1n
-- Applied directly to the live project (bykmyttaesuyjwvtnxks) on 2026-09-06.
--
-- THE root cause of "bridges are never created": staff_create_task
-- declares RETURNS TABLE(task_id uuid, task_number text, bridge_id uuid,
-- bridge_number text). In PL/pgSQL, every name in a RETURNS TABLE(...)
-- list becomes an implicit OUT-parameter variable inside the function
-- body. The bridges INSERT's
--   RETURNING id, bridge_number INTO v_bridge_id, v_bridge_number
-- therefore had "bridge_number" ambiguous between that implicit OUT
-- parameter and the bridges.bridge_number column — Postgres rejected the
-- statement with 42702 EVERY time, meaning every cross-department task
-- creation failed at the Bridge insert step (confirmed by direct test:
-- reproduced the exact 42702 error pre-fix, then confirmed a real
-- task+bridge pair was created post-fix, in a rolled-back transaction).
-- Same-department tasks never touch this code path, which is why those
-- always worked and masked the bug. The staff_tasks INSERT just above it
-- already avoided this exact trap by aliasing the insert
-- (`AS inserted_task` + `inserted_task.id`/`.task_number`) — this applies
-- the identical fix to the bridges INSERT (`AS inserted_bridge` +
-- `inserted_bridge.id`/`.bridge_number`).
--
-- This was already broken before this session touched the project — not a
-- regression from any earlier migration in this series.

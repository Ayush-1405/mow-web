-- AssignTask.jsx's form always shows Quantity + Requirement/Bridge Note,
-- regardless of whether the task is actually a cross-department Bridge —
-- but staff_create_task only ever saved them (into the bridges table)
-- when v_is_bridge was true. For a same-department task, whatever the
-- creator typed into those two fields was silently discarded — visible
-- nowhere, not even to the creator on reload. Adding both columns
-- directly to staff_tasks so they're saved and shown for EVERY task; the
-- existing bridges.requirement_text/quantity insert is untouched (mirrors
-- the same value for Bridges.jsx's own cross-department view).
ALTER TABLE public.staff_tasks ADD COLUMN IF NOT EXISTS requirement_text text;
ALTER TABLE public.staff_tasks ADD COLUMN IF NOT EXISTS quantity text;

-- Previously only Management/Dept Head could set ANY verifier other than
-- themselves; a plain employee/supervisor creating a task could only ever
-- verify their own work. Now they may ALSO specifically pick their own
-- Department Head (staff_user_dept_in_hod_scope against their OWN
-- from-department, not an arbitrary one) — not any other user, so this
-- stays narrow rather than reopening the "pick any verifier" gate.
-- staff_list_assignable_users_all() already returns the caller's dept
-- head in the picker (confirmed: employee/supervisor see every active
-- non-confidential department's roster) — this was purely a server-side
-- rejection, not a missing UI option.
